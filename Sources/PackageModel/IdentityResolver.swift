...
public protocol IdentityResolver {
    ...
    func resolveIdentity(for url: GitURL) throws -> PackageIdentity
...
public struct DefaultIdentityResolver: IdentityResolver {
    ...
    public func resolveIdentity(for url: GitURL) throws -> PackageIdentity {
        ...
        if let path = try? AbsolutePath(validating: location) {
            ...
            return PackageIdentity(url: GitURL(location))
        }
    }
