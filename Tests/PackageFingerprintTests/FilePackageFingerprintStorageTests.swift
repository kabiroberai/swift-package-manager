...
final class FilePackageFingerprintStorageTests: XCTestCase {
    func testHappyCase() throws {
        ...
        let sourceControlURL = GitURL("https://example.com/mona/LinkedList.git")
        ...
        do {
            ...
            XCTAssertEqual(registryFingerprints?[.sourceCode]?.origin.url, GitURL(registryURL))
    ...
    func testDifferentRepoURLsThatHaveSameIdentity() throws {
        ...
        let directoryPath = AbsolutePath("/fingerprints")
        let storage = FilePackageFingerprintStorage(fileSystem: mockFileSystem, directoryPath: directoryPath)
        let fooURL = GitURL("https://example.com/foo/LinkedList.git")
        let barURL = GitURL("https://example.com/bar/LinkedList.git")
    ...
    func testFingerprintsOfDifferentContentTypes() throws {
        ...
        XCTAssertEqual(registryFingerprints?[.manifest(.none)]?.origin.url, GitURL(registryURL))
        XCTAssertEqual(registryFingerprints?[.manifest(.none)]?.value, "manifest-checksum-1.0.0")
        XCTAssertEqual(registryFingerprints?[.manifest(ToolsVersion.v5_6)]?.origin.url, GitURL(registryURL))
